...
public class TeamSystemWebAccessBrowserTest {
    ...
    @Test public void assertChangeSetLinkUsesScmConfiguration() throws Exception {
        ...
        when(project.getScm()).thenReturn(new TeamFoundationServerScm("http://server:80", null, null, null, false, null, null, (Secret) null, true, false));
    ...
    @Test public void assertChangeSetLinkUsesScmConfigurationRestUrl() throws Exception {
        ...
        AbstractProject<?,?> project = mock(AbstractProject.class);
        when(build.getProject()).thenReturn(project);
        when(project.getScm()).thenReturn(new TeamFoundationServerScm("http://server:80", null, null, null, false, null, null, (Secret) null, true, true));
