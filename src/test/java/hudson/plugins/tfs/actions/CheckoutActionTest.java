...
public class CheckoutActionTest {
    ...
    public void assertFirstCheckoutBySingleVersionSpecNotUsingUpdate() throws Exception {
    	...
    	when(workspaces.exists("workspace", true)).thenReturn(true).thenReturn(false);
        when(workspaces.newWorkspace(eq("workspace"), eq("project"), eq(EMPTY_CLOAKED_PATHS_LIST), isA(String.class))).thenReturn(workspace);
    	when(workspaces.getWorkspace("workspace", true)).thenReturn(workspace);
    	
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, ".", false, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertFirstCheckoutNotUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, ".", false, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertFirstCheckoutBySingleVersionSpecUsingUpdate() throws Exception {
        ...
        when(server.getProject("project")).thenReturn(project);
        when(project.getProjectPath()).thenReturn("project");
        when(workspaces.exists(new Workspace("workspace"), true)).thenReturn(false);
        when(workspaces.newWorkspace(eq("workspace"), eq("project"), eq(EMPTY_CLOAKED_PATHS_LIST), isA(String.class))).thenReturn(workspace);
        
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, ".", true, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertFirstCheckoutUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, ".", true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertSecondCheckoutBySingleVersionSpecUsingUpdate() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(server.getProject("project")).thenReturn(project);
        when(workspaces.exists("workspace", true)).thenReturn(true);
        when(workspaces.getWorkspace("workspace", true)).thenReturn(workspace);
    ...
    public void assertDetailedHistoryIsRetrievedInSecondBuildCheckingOutByLabel() throws Exception {
        ...
        CheckoutAction action = new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, ".", true, true);
    ...
    public void assertWorkFolderIsCleanedIfNotUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, "tfs-ws", false, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertWorkFolderIsCleanedIfNotUsingUpdateCheckingOutByLabel() throws Exception {
        ...
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, "tfs-ws", false, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertWorkspaceIsNotCleanedIfUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", EMPTY_CLOAKED_PATHS_LIST, "tfs-ws", true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertCheckoutBySingleVersionSpecDeletesWorkspaceAtStartIfNotUsingUpdate() throws Exception {
        ...
        verify(workspaces, times(2)).exists("workspace", true);
    ...
    public void assertCheckoutDoesNotDeleteWorkspaceAtStartIfUsingUpdate() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(workspaces.exists("workspace", true)).thenReturn(true).thenReturn(true);
        when(workspaces.getWorkspace("workspace", true)).thenReturn(workspace);
        when(server.getProject("project")).thenReturn(project);
    ...
    public void assertCheckoutDoesNotDeleteWorkspaceIfNotUsingUpdateAndThereIsNoWorkspace() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(workspaces.exists("workspace", true)).thenReturn(false).thenReturn(false);
        when(workspaces.newWorkspace(eq("workspace"), eq("project"), eq(EMPTY_CLOAKED_PATHS_LIST), isA(String.class))).thenReturn(workspace);
        when(server.getProject("project")).thenReturn(project);
        when(project.getProjectPath()).thenReturn("project");
